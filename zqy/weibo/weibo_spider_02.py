...
url = 'https://m.weibo.cn/status/4138566579839305'
...
def get_basic_info():
    ...
    reproduced_num = driver.find_element_by_class_name('comment-tab').text
    comment_num = driver.find_element_by_class_name('comment-tab-cur').text
    favor_num = driver.find_element_by_class_name('comment-top-right').text
    brief_introduction = '链接网址: ' + url + '\n' + '用户名: '+ name + '\n' + '用户设备: '+ device + '\n' +\
        '发布时间： ' + push_time + '\n' + '微博内容: '+ weibo_text + '\n' + '转载数: ' + reproduced_num + '\n' +\
                         '评论数: ' + comment_num + '\n'+ '点赞数: '+ favor_num + '\n'
